...
import { addReportSchema, updateReportSchema } from "@/app/types/ZodSchema";
...
export async function updateReport(prevState: any, formData: FormData) {
  ...
  if (buktiFile instanceof File && buktiFile.size === 0) {
    validatedFields = updateReportSchema.safeParse({
      nama_program: formData.get("nama_program"),
      jml_penerima: parseInt(String(formData.get("jml_penerima"))),
      provinsi: formData.get("provinsi"),
      kabupaten_kota: String(formData.get("kabupaten_kota")),
    ...
    validatedFields = addReportSchema.safeParse({
      nama_program: formData.get("nama_program"),
      jml_penerima: parseInt(String(formData.get("jml_penerima"))),
      provinsi: formData.get("provinsi"),
      kabupaten_kota: String(formData.get("kabupaten_kota")),
  ...
  newReportData.append("tgl_penyaluran", validatedFields.data.tgl_penyaluran);
  newReportData.append("catatan", validatedFields.data.catatan || "");
  if (validatedFields.data.bukti) {
    newReportData.append("bukti", validatedFields.data.bukti);
  }

  const response = await reportsService.updateReport(id, newReportData);
  if (!response) {
    return {
      ...prevState,
      isLoading: false,
